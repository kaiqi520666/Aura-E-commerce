...
onMounted(async () => {
  ...
  try {
    ...
    toast.error(error.message || 'Unable to load your account.')
  } finally {
    syncProfileForm()
  ...
  <section class="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
    <template v-if="initialLoading">
      <div class="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div class="space-y-4">
          ...
          <div class="skeleton-block h-13 w-60 rounded-2xl"></div>
        </div>
      ...
      <div class="grid gap-8 lg:grid-cols-2">
        <section class="skeleton-panel p-6">
          ...
          <div class="grid gap-4">
            ...
            <div class="skeleton-block h-14 rounded-[1.2rem]"></div>
            <div class="skeleton-block h-14 rounded-[1.2rem]"></div>
            <div class="skeleton-block h-28 rounded-[1.2rem]"></div>
      ...
      <section class="skeleton-panel mt-10 p-6">
        <div class="mb-6 space-y-4">
          <div class="skeleton-block skeleton-line w-28"></div>
          <div class="skeleton-block h-11 w-56 rounded-2xl"></div>
        </div>
        ...
        <div class="grid gap-8 lg:grid-cols-[1fr_360px]">
          <div class="grid gap-4">
            <article
              v-for="index in 2"
              :key="index"
              class="rounded-[1.1rem] border border-[var(--color-border)] p-5"
            >
              <div class="space-y-3">
          ...
          <div class="rounded-[1.1rem] border border-[var(--color-border)] bg-[var(--color-cream)] p-5">
            ...
            <div class="grid gap-3">
              ...
              <div class="skeleton-block h-14 rounded-[1.2rem]"></div>
              <div class="skeleton-block h-14 rounded-[1.2rem]"></div>
              <div class="skeleton-block h-5 w-32 rounded-full"></div>
              <div class="skeleton-block h-12 rounded-full"></div>
            </div>
          </div>
        </div>
    ...
    <template v-else>
      <div class="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div class="flex flex-wrap gap-3">
          <button class="primary-pill" type="button" @click="router.push('/orders')">
            View orders
          </button>
          <button class="secondary-pill" type="button" @click="signOut">Logout</button>
        </div>
      </div>
      ...
      <div class="grid gap-8 lg:grid-cols-2">
        ...
        >
          ...
          <div class="grid gap-4">
            ...
            ></textarea>
            <button class="primary-pill justify-center" type="button" @click="saveProfile">
              Save profile
            </button>
          </div>
        </section>
        ...
        >
          ...
          <div class="grid gap-4">
            ...
            <input
              ...
              type="password"
            />
            <button class="secondary-pill justify-center" type="button" @click="savePassword">
              Update password
            </button>
          </div>
        </section>
            ...
            >
              <div class="flex items-start justify-between gap-4">
                ...
                <div class="flex gap-2">
                  <button
                    class="secondary-pill !px-4 !py-2"
                    type="button"
          ...
          <div class="rounded-[1.1rem] border border-[var(--color-border)] bg-[var(--color-cream)] p-5">
            ...
            <div class="grid gap-3">
              ...
              <label class="flex items-center gap-2 text-sm text-[var(--color-muted)]">
                <input v-model="addressForm.isDefault" type="checkbox" /> Set as default
              </label>
              <button class="primary-pill justify-center" type="button" @click="saveAddressItem">
                Save address
              </button>
            </div>
          </div>
